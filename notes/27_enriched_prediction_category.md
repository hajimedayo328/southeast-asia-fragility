...
- ✅ **backbone のノード値(TrustHub レベル)はデータ化済み (2026-06-06)**。
  `src/h_petri/data/backbone_facts.py`: Bakong/PayNow/KBZPay/GCash の運営主体・規制
  ステータスを**出典付きで確認**し、「法的裏付け tier → Heyting レベル」の明示ルールで導出。
  4つとも従来の手置き値と一致 → **核となる backbone 分類は (b) 著者割当て から
  (c) 出典付き事実+明示ルール に格上げ**。
- ⚠️ ただし enriched ペアの**辺(影響強度)の hom 値**はまだ著者割当て(次の宿題)
